  ...
  return (
    <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500 w-full max-w-[1280px] mx-auto pb-8">
        ...
        <div className="h-40 sm:h-48 relative">
          ...
          <div className="absolute bottom-0 left-0 right-0 p-4 sm:p-5 space-y-3">
            <div className="flex items-center gap-2 flex-wrap">
              {journey.category && (
                <span className="bg-white/20 backdrop-blur-md text-white text-[9px] font-semibold uppercase tracking-wider px-2 py-0.5 rounded border border-white/20">
                  {journey.category}
                </span>
              )}
              {isMastered && (
                <span className="flex items-center gap-1 bg-emerald-500/95 backdrop-blur-sm text-white text-[9px] font-semibold uppercase tracking-wider px-2 py-0.5 rounded shadow-sm">
            ...
            <div className="max-w-md space-y-1.5">
              ...
              <div className="h-1.5 bg-white/20 rounded overflow-hidden backdrop-blur-sm">
                <div
                  className={`h-full ${isMastered ? 'bg-emerald-400' : 'bg-white'} transition-all duration-700 rounded`}
                  style={{ width: `${pct}%` }}
                />
              </div>
            </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        <div className="sm:col-span-2 bg-white border border-slate-100 rounded-lg p-4 shadow-sm flex flex-col justify-between space-y-3">
          <div className="space-y-1.5">
            <h2 className="text-xs font-semibold uppercase tracking-wider text-slate-400">About This Journey</h2>
            <p className="text-xs text-slate-600 leading-relaxed font-medium">{journey.description}</p>
          </div>
          {journey.topics && journey.topics.length > 0 && (
            <div className="flex flex-wrap gap-1.5 pt-1">
              {journey.topics.map((topic, i) => (
                <span key={i} className={`text-[9px] font-semibold uppercase tracking-wider px-2 py-0.5 rounded border ${theme.bg} ${theme.border} ${theme.accent}`}>
                  {topic}
                </span>
              ))}
            </div>
        ...
        <div className="bg-white border border-slate-100 rounded-lg p-4 shadow-sm space-y-3">
          <h3 className="text-xs font-semibold uppercase tracking-widest text-slate-400">Stats</h3>
          <div className="space-y-2.5">
            <div className="flex items-center gap-2.5">
              ...
              <div>
                <p className="text-[10px] font-medium text-slate-400">Episodes</p>
                <p className="text-xs font-semibold text-slate-800 leading-none">{totalCount}</p>
              </div>
            </div>
            <div className="flex items-center gap-2.5">
              ...
              <div>
                <p className="text-[10px] font-medium text-slate-400">Total XP</p>
                <p className="text-xs font-semibold text-slate-800 leading-none">{journey.totalXP}</p>
              </div>
            </div>
            {completedCount > 0 && (
              <div className="flex items-center gap-2.5">
                ...
                <div>
                  <p className="text-[10px] font-medium text-slate-400">XP Earned</p>
                  <p className="text-xs font-semibold text-emerald-600 leading-none">{xpEarned}</p>
                </div>
              </div>
            )}
          </div>
      ...
      <div className="bg-white border border-slate-100 rounded-lg p-4 sm:p-5 shadow-sm">
        <div className="flex items-center justify-between mb-6 pb-4 border-b border-slate-100">
          <div>
            <h2 className="text-xs font-semibold uppercase tracking-wider text-slate-800 flex items-center gap-1.5">
              <Sparkles className="text-purple-500" size={16} /> Path to Mastery
            </h2>
            <p className="text-[11px] font-medium text-slate-400 mt-0.5">Complete each activity in order to unlock the next</p>
          </div>
            ...
            return (
              <div
                ...
                className={`relative p-4 rounded-lg border transition-all ${isCompleted
                    ? 'bg-emerald-50/20 border-emerald-100 cursor-pointer hover:border-emerald-200'
                    : isUnlocked
                ...
                <div className="flex items-center gap-4">
                  ...
                  >
                    {isCompleted ? (
                      ...
                      <span className={`text-xs font-semibold ${theme.accent}`}>{index + 1}</span>
                    ) : (
                  ...
                  <div className="flex-1 min-w-0">
                    <p className={`text-[9px] font-semibold uppercase tracking-widest mb-0.5 ${isCompleted ? 'text-emerald-500' : isUnlocked ? theme.accent : 'text-slate-400'
                      }`}>
                      Activity {index + 1}
                    </p>
                    <h4 className={`text-xs sm:text-sm font-semibold leading-tight ${isUnlocked ? 'text-slate-800' : 'text-slate-400'
                      }`}>
                      {episode.title}
                    </h4>
                    {isUnlocked && (
                      <div className="flex items-center gap-1 mt-1">
                        ...
                        <span className="text-[10px] font-medium text-slate-400">{episode.points} Points</span>
                      </div>
                    )}
                  </div>
                  ...
                  <div className="shrink-0">
                    {isCompleted ? (
                      ...
                      <div className={`w-7 h-7 rounded-lg bg-gradient-to-br ${theme.gradient} flex items-center justify-center shadow-sm`}>
        ...
        {isMastered && (
          <div className="mt-6 p-4 bg-emerald-50/30 border border-emerald-100 rounded-lg text-center space-y-1.5">
            <div className="text-2xl">🎉</div>
            <h3 className="text-xs font-semibold text-emerald-800">Journey Mastered!</h3>
